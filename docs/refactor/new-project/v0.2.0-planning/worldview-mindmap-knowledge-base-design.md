...
- DOM 结构是中心 root node + 左右一级世界节点 + connector div。
- 片段中未出现 React Flow、G6、X6、Mind Elixir、jsMind、markmap 的典型容器、脚本引用、canvas 节点或 nodes / edges 数据结构。

因此，竞品这页应理解为「自研的世界构建功能树 / 脑图式导航组件」，不是完整的可自由编辑 mindmap canvas。它的价值主要在信息架构：把世界观拆成更专业的创作节点。

v0.2 demo 固定一级世界节点：

| 一级节点 | 覆盖内容 |
| --- | --- |
| 规则体系 | 世界运行规则、能力边界、代价约束 |
| 地理地图 | 地理格局、地点、路线、地图资源 |
| 人物档案 | 核心人物档案、身份背景、人物弧光 |
| 势力阵营 | 组织、阵营、政治经济结构 |
| 关系网络 | 人物或势力之间的从属、合作、敌对、情感和动态关系 |
| 风俗文化 | 信仰、语言、风俗、艺术娱乐 |
| 矛盾冲突 | 主要矛盾、长期冲突、剧情压力 |
| 事件年表 | 历史事件、关键转折、事件因果 |
| 伏笔 | 伏笔线索、埋设位置、回收条件 |
...
对比后判断：固定世界节点不再拆成一级系统和二级模块，而是直接采用上述 10 个一级节点，减少用户需要维护的固定节点数。其中「人物档案」维护人物实体资料，「关系网络」排列在「势力阵营」之后，用于统一维护人物或势力之间的关系。底层仍保留人物、关系、势力、地点、规则、伏笔、事件等实体类型，避免导航层复杂度向用户暴露。
...
百科仓库目录与导图固定一级节点保持一致，不再额外拆二级 / 三级固定目录：
...
- 主画布：世界观根节点、9 个固定一级世界节点、用户自增节点、关系线。画布区域需要支持纵向滚动，并预留大于首屏高度的内部画布空间。一级固定节点左右分布，用户自增节点从对应一级节点继续展开，避免固定节点过多导致编辑负担。
...
4. 百科仓库展示由导图节点派生的一级世界节点和示例条目，固定节点覆盖规则体系、地理地图、人物档案、势力阵营、关系网络、风俗文化、矛盾冲突、事件年表、伏笔、灵感。
...
- 固定世界节点只保留一级：规则体系、地理地图、人物档案、势力阵营、关系网络、风俗文化、矛盾冲突、事件年表、伏笔、灵感。
- 点击「百科仓库」可查看与导图一级节点一致的世界节点目录。
...
- 思维导图中的固定业务节点不能删除，只允许清空内容。固定业务节点包括规则体系、地理地图、人物档案、势力阵营、关系网络、风俗文化、矛盾冲突、事件年表、伏笔、灵感。该策略用于减少删除固定目录后带来的百科条目失联、校验路径失效和演示流程断裂问题。
